...
function create_versandetikett(frm) {
	frappe.call({
		method: "versand_integration.api.create_shipment_from_delivery_note",
		args: { delivery_note: frm.doc.name, create_label: 1 },
		freeze: true,
		freeze_message: __("Versandetikett wird bei DHL erstellt …"),
		callback: (r) => {
			...
			frm.reload_doc();
			if (res.label_file) {
				window.open(res.label_file, "_blank");
			} else {
				frappe.set_route("Form", "Versandsendung", res.name);
			}
		},
	});
}
